...
export const Step4_Generation: React.FC<Props> = ({ data, onReset, session, onBack, onUpdate }) => {
  ...
  useEffect(() => {
    const genAndSave = async () => {
      // Se já existe prompt gerado no estado global, não gera de novo
      ...
      setPrompt(result);
      onUpdate({ generatedPrompt: result }); // Salva no estado global do Wizard
    ...
    genAndSave();
  }, [data.generatedPrompt]); // Dependência ajustada para evitar loops
